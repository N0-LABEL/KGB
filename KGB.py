...
async def on_member_remove(member):
    """Срабатывает при кике или выходе пользователя"""
    # Не обрабатываем события с основного сервера
    if member.guild.id == MAIN_SERVER_ID:
        return
    
    # Проверяем наличие защитной роли
    if not await has_protected_role(member):
        return
    
    # Проверяем, был ли это кик (а не просто выход)
    try:
        # Пытаемся получить аудит-лог
        async for entry in member.guild.audit_logs(limit=1, action=discord.AuditLogAction.kick):
            if entry.target.id == member.id:
                # Это был кик - отправляем приглашение
                try:
                    invite = await member.guild.text_channels[0].create_invite(
                        max_age=0,  # Бессрочное приглашение
                        reason="Автоматическое приглашение для кикнутого участника"
                    )
                    await send_dm(
                        member,
                        f"⚠️ Вас исключили с сервера **{member.guild.name}**\n"
                        f"🔗 Вот бессрочная ссылка для возврата: {invite.url}\n"
                        f"Своих не бросаем!"
                    )
                except Exception as e:
        ...
        print(f"Нет прав на просмотр аудит-лога на сервере {member.guild.name}")
    except Exception as e:
...
async def on_member_ban(guild, user):
    ...
    # Не обрабатываем баны на основном сервере
    ...
    try:
        ...
        # Создаем бессрочное приглашение
        ...
        await send_dm(
            ...
            f"🔗 Вот бессрочная ссылка для входа: {invite.url}\n"
            f"Своих не бросаем!"
        )
